...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app agent_eta db upgrade
	@echo ""
	@echo "🚀 Dev stack ready:"
	@echo "   API:      http://localhost:8500"
	@echo "   Docs:     http://localhost:8500/docs"
	@echo "   Admin:    http://localhost:8500/admin"
	@echo "   Frontend: http://localhost:3500  (run 'make dev-frontend' or 'cd frontend && bun dev')"
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		agent_eta user list 2>/dev/null \
		...
		docker compose -f docker-compose.dev.yml exec -T app \
			agent_eta user create \
				--email admin@example.com --password admin123 --superuser \
		&& echo "  ✅ Admin created. Login at http://localhost:8500/admin"; \
	fi
...
dev-frontend:
	...
	@echo ""
	@echo "✅ Frontend at http://localhost:3500  (backend must be up — 'make dev')"

# === Staging: built images, no bind mounts (production-like, local DB) ===
stage:
	docker compose -f docker-compose.yml up -d --build
	$(call _wait_for_db,docker-compose.yml)
	docker compose -f docker-compose.yml exec -T app agent_eta db upgrade
	@echo "✅ Staging stack at http://localhost:8500"
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app agent_eta db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run agent_eta db migrate -m "initial" || true
	cd backend && uv run agent_eta db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend agent_eta db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend agent_eta db upgrade

db-downgrade:
	uv run --directory backend agent_eta db downgrade

db-current:
	uv run --directory backend agent_eta db current

db-history:
	uv run --directory backend agent_eta db history
...
routes:
	uv run --directory backend agent_eta server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend agent_eta user create-admin

user-create:
	uv run --directory backend agent_eta user create

user-list:
	uv run --directory backend agent_eta user list

# === Celery ===
celery-worker:
	uv run --directory backend agent_eta celery worker

celery-beat:
	uv run --directory backend agent_eta celery beat

celery-flower:
	uv run --directory backend agent_eta celery flower
	@echo ""
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/agent_eta"
...
help:
	@echo ""
	@echo "agent_eta - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run agent_eta rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run agent_eta rag-search <query> -c <collection>  Search"
	@echo "  uv run agent_eta rag-collections                     List collections"
	@echo "  uv run agent_eta rag-sources                         List sync sources"
	@echo "  uv run agent_eta rag-source-add                      Add sync source"
	@echo "  uv run agent_eta rag-source-sync <id>                Trigger sync"
	@echo ""
	@echo "Celery:"
