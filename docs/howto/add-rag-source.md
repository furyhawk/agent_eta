...
- **CLI** — `uv run agent_eta rag-sync` commands
...
uv run agent_eta cmd rag-sync --connector my_source \
...
# Trigger a sync
curl -X POST http://localhost:8500/api/v1/rag/sync/sources/{source_id}/sync \
    -H "Authorization: Bearer $TOKEN"
```
