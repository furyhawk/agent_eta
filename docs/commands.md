...
```bash
cd backend
uv run agent_eta <group> <command> [options]
```

### Server Commands

```bash
uv run agent_eta server run              # Start dev server
uv run agent_eta server run --reload     # With hot reload
uv run agent_eta server run --port 9000  # Custom port
uv run agent_eta server routes           # Show all registered routes
```
...
```bash
uv run agent_eta db init                  # Run all migrations
uv run agent_eta db migrate -m "message"  # Create new migration
uv run agent_eta db upgrade               # Apply pending migrations
uv run agent_eta db upgrade --revision e3f  # Upgrade to specific revision
uv run agent_eta db downgrade             # Rollback last migration
uv run agent_eta db downgrade --revision base  # Rollback to start
uv run agent_eta db current               # Show current revision
uv run agent_eta db history               # Show migration history
```
...
uv run agent_eta user create

# Create user non-interactively
uv run agent_eta user create --email user@example.com --password secret

# Create user with specific role
uv run agent_eta user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run agent_eta user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run agent_eta user create-admin --email admin@example.com --password secret

# Change user role
uv run agent_eta user set-role user@example.com --role admin

# List all users
uv run agent_eta user list
```

### Celery Commands

```bash
uv run agent_eta celery worker                    # Start worker
uv run agent_eta celery worker --loglevel debug   # Debug logging
uv run agent_eta celery worker --concurrency 8    # 8 worker processes
uv run agent_eta celery beat                      # Start scheduler
uv run agent_eta celery flower                    # Start Flower UI
uv run agent_eta celery flower --port 5556        # Custom Flower port
```
...
```bash
uv run agent_eta cmd <command-name> [options]
```
...
uv run agent_eta cmd rag-ingest ./docs/guide.pdf

# Ingest a directory
uv run agent_eta cmd rag-ingest ./docs/

# Ingest recursively into a specific collection
uv run agent_eta cmd rag-ingest ./docs/ --collection knowledge --recursive

# Ingest with sync mode
uv run agent_eta cmd rag-ingest ./docs/ --sync-mode new_only
uv run agent_eta cmd rag-ingest ./docs/ --sync-mode update_only

# Skip replacing existing documents
uv run agent_eta cmd rag-ingest ./docs/ --no-replace
```
...
uv run agent_eta cmd rag-search "what is fastapi"

# Search a specific collection
uv run agent_eta cmd rag-search "deployment guide" --collection docs

# Get more results
uv run agent_eta cmd rag-search "deployment" --top-k 10
```
...
uv run agent_eta cmd rag-collections

# Show overall RAG system statistics
uv run agent_eta cmd rag-stats

# Drop a collection (with confirmation)
uv run agent_eta cmd rag-drop my_collection

# Drop without confirmation
uv run agent_eta cmd rag-drop my_collection --yes
```
...
uv run agent_eta cmd rag-sync-gdrive --collection docs

# Sync from a specific folder
uv run agent_eta cmd rag-sync-gdrive --collection docs --folder-id abc123
```
...
uv run agent_eta cmd rag-sync-s3 --collection docs

# Sync from a specific prefix (folder)
uv run agent_eta cmd rag-sync-s3 --collection docs --prefix documents/

# Sync from a specific bucket
uv run agent_eta cmd rag-sync-s3 --collection docs --bucket my-bucket
```
...
uv run agent_eta cmd rag-source-remove <source-id>
uv run agent_eta cmd rag-source-remove <source-id> --yes  # Skip confirmation

# Trigger sync for a specific source
uv run agent_eta cmd rag-source-sync <source-id>

# Trigger sync for all active sources
uv run agent_eta cmd rag-source-sync --all
```
...
```bash
uv run agent_eta cmd my-command --name test
```
