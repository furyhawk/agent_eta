...
|----------|---------|-------------|
| `PROJECT_NAME` | `agent_eta` | Display name for the project |
...
| `GOOGLE_REDIRECT_URI` | `http://localhost:8500/api/v1/oauth/google/callback` | OAuth2 callback URL |
| `FRONTEND_URL` | `http://localhost:3500` | Frontend URL for OAuth2 redirects |
...
| `POSTGRES_DB` | `agent_eta` | Database name |
...
| `LOGFIRE_SERVICE_NAME` | `agent_eta` | Service name in Logfire dashboard |
...
| `S3_RAG_BUCKET` | `agent_eta-rag` | Bucket name |
...
| `S3_BUCKET` | `agent_eta` | Bucket name |
