---
name: rag-knowledge
description: Work with the RAG knowledge base — ingest documents, run semantic search, manage collections, or add a sync source/connector (Google Drive, S3). Use when populating or debugging the knowledge base, tuning retrieval, or adding a new document source. This project uses milvus + openai embeddings.
---

# RAG Knowledge Base (milvus)
...
```bash
uv run agent_eta cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run agent_eta cmd rag-search "your question" --collection docs        # semantic search
uv run agent_eta cmd rag-collections                                     # list collections
uv run agent_eta cmd rag-stats                                           # chunk/vector counts
uv run agent_eta cmd rag-drop <collection> --yes                         # delete a collection
```

Ingestion = parse → chunk → embed → upsert into milvus. Re-ingesting the same source updates it (use `--no-replace` / `--sync-mode` to control dedupe).
...
```bash
uv run agent_eta cmd rag-sources                  # list
uv run agent_eta cmd rag-source-add               # add (interactive)
uv run agent_eta cmd rag-source-sync --all        # trigger a sync
```
