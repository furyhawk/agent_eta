...
| **Cache** | Redis |
| **AI Framework** | pydantic_deep (openrouter) |
...
- Admin: <http://localhost:8500/admin> — `admin@example.com` / `admin123` after `make seed`
- Frontend: <http://localhost:3500> — start with `make dev-frontend` (Docker) or `cd frontend && bun install && bun dev` (local)
...
└── commands/             # Click CLI commands (auto-discovered by `agent_eta cmd …`)
...
The generated project ships a Click CLI exposed as `agent_eta` (after `make install`):

```bash
agent_eta server run --reload          # dev server
agent_eta db upgrade                   # apply migrations
agent_eta db migrate -m "message"      # create new migration
agent_eta user create-admin            # interactive admin creation
agent_eta rag-ingest <path> -c docs    # ingest local files
agent_eta rag-search "query" -c docs   # semantic search
agent_eta rag-collections              # list collections
agent_eta celery worker                # start worker
agent_eta celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `agent_eta --help` for full CLI docs.
...
agent_eta rag-ingest /path/to/docs/ --collection documents --recursive
# Pull from Google Drive (service-account auth)
agent_eta rag-sync-gdrive --collection documents --folder-id <id>
# Pull from S3 / MinIO
agent_eta rag-sync-s3 --collection documents --prefix docs/

# Semantic search
agent_eta rag-search "your query" --collection documents
```
